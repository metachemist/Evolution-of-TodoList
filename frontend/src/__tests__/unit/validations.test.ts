...
describe('taskCreateSchema', () => {
  ...
  it('accepts title with description', () => {
    const result = taskCreateSchema.safeParse({ title: 'My task', description: 'Details here' })
    expect(result.success).toBe(true)
  })

  it('accepts empty string for description', () => {
    const result = taskCreateSchema.safeParse({ title: 'My task', description: '' })
    expect(result.success).toBe(true)
  })
  ...
  it('rejects description exceeding 5000 chars', () => {
    const result = taskCreateSchema.safeParse({ title: 'Task', description: 'a'.repeat(5001) })
    expect(result.success).toBe(false)
    if (!result.success) {
...
describe('taskUpdateSchema', () => {
  it('is the same schema as taskCreateSchema', () => {
    ...
    const createResult = taskUpdateSchema.safeParse({ title: 'Updated title' })
